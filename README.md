...
> **PowerShell scripts for Active Directory security auditing, endpoint hardening, and infrastructure risk reduction.**
...
This toolkit is a collection of production-grade PowerShell scripts built for Windows enterprise environments. The scripts cover Active Directory certificate services (ADCS) auditing, service account privilege analysis, Group Policy Object (GPO) hardening reviews, share permission enumeration, and endpoint agent lifecycle management.

All audit scripts are **read-only** — they make no changes to Active Directory, Group Policy, or any system configuration. Output is written to CSV and optionally to HTML reports.

Designed to support security teams performing internal assessments, compliance reviews, and hardening initiatives aligned with NIST 800-53, MITRE ATT&CK, CIS Controls v8, DISA STIGs, and SpecterOps ESC research.
...
| [`Search-GPOsBySecurityGroup.ps1`](#search-gposbysecuritygroupps1) | Scans all domain GPOs for security group references — security filtering, delegation, and XML settings |
...
| Permissions | Domain-joined machine with AD read access; some scripts require Domain Admin or delegated read-only auditor rights |
...
| MITRE ATT&CK | T1649, T1222.001, T1484, T1558.003 |
...
- All **audit scripts are read-only** and make no changes to Active Directory, Group Policy, or any system
